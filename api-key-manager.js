...
const ApiKeyManager = (function() {
  const STORAGE_PREFIX = 'biolink-ia_apikey_';
  ...
  const ENABLED_SERVICES = ['openrouter', 'openai']; // texto: OpenRouter (1 chave → todos) + OpenAI nativo
  ...
  // Refreshed from the LIVE OpenRouter catalog 2026-09-16 — every id cross-checked
  // against GET /api/v1/models (COR-048). Text-only tool (writes bio/link copy).
  const AI_MODELS = {
    claude: {
    ...
    trending: {
      ...
      // Live OpenRouter ranking 2026-09-16 (ids validated).
      models: [
