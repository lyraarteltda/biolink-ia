    ...
    modal.innerHTML =
      ...
      '<div style="position:relative;background:#13161f;border:1px solid rgba(255,255,255,.08);border-radius:20px;padding:28px;width:90%;max-width:440px;">' +
        ...
        '<div style="display:flex;gap:8px;margin-bottom:16px;">' +
          ...
          '<button class="fb-type-btn active" data-type="feature" style="flex:1;padding:10px;background:rgba(255,77,94,.12);border:1px solid rgba(255,77,94,.3);border-radius:10px;color:#ff7683;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;">💡 Sugestão</button>' +
        '</div>' +
        ...
        '<button id="feedback-send" style="width:100%;margin-top:12px;padding:14px;background:linear-gradient(180deg,#ff7683,#ff4d5e);color:#fff;border:none;border-radius:12px;font-size:15px;font-weight:600;cursor:pointer;font-family:inherit;">' +
    ...
    modal.querySelectorAll('.fb-type-btn').forEach(function(b) {
      b.addEventListener('click', function() {
        modal.querySelectorAll('.fb-type-btn').forEach(function(x) {
          x.style.background = x.dataset.type === 'bug' ? 'rgba(239,68,68,.05)' : 'rgba(255,77,94,.05)';
          x.style.borderColor = 'rgba(255,255,255,.08)';
        });
        b.style.background = b.dataset.type === 'bug' ? 'rgba(239,68,68,.1)' : 'rgba(255,77,94,.12)';
        b.style.borderColor = b.dataset.type === 'bug' ? 'rgba(239,68,68,.2)' : 'rgba(255,77,94,.3)';
